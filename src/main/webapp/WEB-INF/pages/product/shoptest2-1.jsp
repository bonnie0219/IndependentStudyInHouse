				...
				if(parsedObjinArray.length==0){
					...
					$('#test').prepend("<div style='text-align:center;'>" +"<h3 align='center'>暫無資料</h3>" + "</div>");
			         ...
			         $.each(parsedObjinArray,function(i,n){
			            var tr = "<div style='position: relative; padding:20px;'>" + "<a href='/inhouse/shop/findById/" + n.product_id + "'>" + 
			            		 "<img style='width:350px; height:350px' src='" + n.product_picture + "'>"  +
			                     "<h4 style='padding:0 auto; position:absolute; top:45px; left:60px;' id='" + n.product_name + "'>" + n.product_name + "</h4>" + 
			                     "<a style='padding:0 auto; position:absolute; top:80px; left:60px;'>" + n.product_type + "</a>" +
...
// 	function deleteKeep(product_id){
// 		//debugger;
// 		var member_name = $("#member_name2").val();
// 	     $.ajax({
// 	         url: '/inhouse/shop/delete/' + product_id +'/'+member_name,
// 	         type: 'POST',
// 	         success: function () {
// 	        	Swal.fire('取消收藏成功');
// 	         },
...
// 	 }
	...
	function countByMemberName(){
		//debugger;
		var member_name = $("#member_name").val();
		$.ajax({
			type:"GET",
			dataType:"json",
			url:"/inhouse/shop/findProductByMemberName",
			data:{member_name:member_name},
			dataType:'JSON',
			contentType:'application/json',
			success: function (data){
				console.log("這邊是data"+data);
				var json = JSON.stringify(data, null, 4);
				console.log("這邊是json"+json);
				var parsedObjinArray = JSON.parse(json);
				console.log("這邊是parsedObjinArray"+parsedObjinArray);
				$('#test3').empty("");
				if(parsedObjinArray.length==0){
					//alert("查無資料");
					$('#test3').prepend("<div style='text-align:center;'>" +"<h3 align='center'>暫無資料</h3>" + "</div>");
			       }else{
			         var table = $('#test3');
			         var trr = "<div style='overflow: auto;'>"
			         table.append(trr);
			         $.each(parsedObjinArray,function(i,n){
			        	 console.log(i,n);
			        	 console.log(n.product_id);
			             var tr = "<div style='background-color:#D4E6F8;width:150px;margin:20px auto';display:inline-block;>" +
 			                      "<a>X</a>" +
// 			                      "<a style='display:inline;' onclick='deleteKeep(" + ${n.product_id} +")'>X</a>' +
// 			             		  '<a onclick="deleteKeep("' + '"'+${n.product_id} + '"," '+ ${pageContext.request.userPrincipal.name} + ' ")>X</a>' +
// 			             		  "<a style='display:inline' onclick='deleteKeep("${n.product_id}","${pageContext.request.userPrincipal.name}")'>X</a>" +
	                     		  "<a href='/inhouse/shop/findById/" + n.product_id + "'>" +
	                     		  "<img style='width:100px; height:100px;margin:10px; padding:20px auto;' src='" + n.product_picture + "'>"  +
	                     		  "<h6 align='center' id='" + n.product_name + "'>" + n.product_name + "</h6>" + "</a>" +
	                     		  "</div>";
...
<li class="breadcrumb-item"><a href="/inhouse/shop/queryallbypage/${pageNo}">${TypeName}</a></li>
<li class="breadcrumb-item active" aria-current="page">第${pageNo}頁</li>
</ol>
</nav>
</div>
</section>
...
<h2 class="fs-sm-40 mb-11 text-center">${TypeName}<hr size=3 color=#C2C2FF style="FILTER: alpha(opacity=100,finishopacity=0,style=3)"></h2>
                        ...
                        <div class="card border-0 hover-change-content product">
                            ...
                            <div class="card-img-overlay d-flex py-4 py-sm-5 pl-6 pr-4" >
                                <div class="d-flex flex-column">
                                    ...
                                    <a style="color:black;" class="text-uppercase letter-spacing-05 fs-12"><b>${p.product_type}</b></a>
...
<%-- 											${p.product_price}</h4> --%>
                                ...
                                <div class="ml-auto d-flex flex-column">
                                    <div class="my-auto content-change-vertical">
										...
										<a onclick="error()" data-placement="left" title="Add to favourite"
										    class="add-to-wishlist d-flex align-items-center justify-content-center text-primary bg-white hover-white bg-hover-primary w-45px h-45px rounded-circle mb-2 border">
											<i id="heart" class="far fa-heart"></i></a>
										</c:if>
	...
	<div id="keep" class="canvas-sidebar cart-canvas my-cart-popup" style="overflow: auto;width:200px;">
		<div>
			<div class="card border-0 pt-5 pb-7 h-100">
				...
				<input type="hidden" id="member_name2" value="${pageContext.request.userPrincipal.name}" />
				<div id="test3">
