...
<li class="breadcrumb-item"><a href="/inhouse/portfolio/portfolioView/getall"> 精選專欄 </a></li>
<c:forEach items="${theColumnBean}" var="p">
<li class="breadcrumb-item active" aria-current="page">${p.portfolio_title}</li>
</ol>
</nav>
</div>
</section>
...
onclick = 'articlelike("${portfolio_id}","${pageContext.request.userPrincipal.name}")' >
 <i class="far fa-thumbs-up"></i>																																				
 </button><a class="fs-20" style="margin:10px;
  margin-top: 40px;width:50px;height:50px;" id="liketo${portfolio_id}">${p.portfolio_like}Like</a>
 <input  type="hidden" id="articles${portfolio_id}" value="${p.portfolio_like+1}"></li>
</c:if>
...
<a class="middle "style="border-radius:70%;width:50px;height:50px;padding-left:12px;line-height: 27px; 
 ...
 <i class="far fa-thumbs-up" ></i>																																				
  ...
  margin-top: 30px;width:50px;height:50px;" id="likeit">${p.portfolio_like}Like</a>
 <input  type="hidden" id="article1" value="${p.portfolio_like+1}"></li>
</c:if>
...
<div style="display: flex;
  ...
  flex-wrap: wrap;
  text-align:center;">
<h1>${p.portfolio_title}</h1></div><hr size=3 color=#C2C2FF style="FILTER: alpha(opacity=100,finishopacity=0,style=3)">
<div style="display: flex;
  justify-content: center;
  align-items: center;
  align-content: center;
...
<div id="fb-root"></div>
 ...
 class="fab "><img src="https://livedoor.blogimg.jp/line_tw/imgs/2/9/29691b85-s.png" style="width:30px;height:30px;">Line</i></a></li>
  <li class="list-inline-item mr-4 "><a href="http://pinterest.com/pin/create/link/?url=http://localhost:8080/inhouse/portfolio/portfolioView/${p.portfolio_id}" class="fs-20"><i                                 
                        ...
                        <li class="list-inline-item"><a href="https://twitter.com/share?text=inhouse${p.portfolio_title}&url=http://localhost:8080/inhouse/portfolio/portfolioView/${p.portfolio_id}" class="fs-20"><i 
...
<a href="/inhouse/portfolio/portfolioView/${fp.portfolio_id}" class="fs-14 mr-4"><i class="fas fa-arrow-left"></i></a>
<a href="/inhouse/portfolio/portfolioView/${fp.portfolio_id}" class="w-70px d-block mr-2"
style="display:inline-block;width:100%;height:50px;overflow:hidden;">
<img src="${fp.portfolio_image}"
...
<a href="/inhouse/portfolio/portfolioView/${fp.portfolio_id}" class="font-weight-bold">
${fp.portfolio_title}
</a>
</div>
</div>
...
<a href="/inhouse/portfolio/portfolioView/${fn.portfolio_id}" class="font-weight-bold">
${fn.portfolio_title}
</a>
</div>
<a href="/inhouse/portfolio/portfolioView/${fn.portfolio_id}" class="w-70px d-block ml-2"
style="display:inline-block;width:100%;height:50px;overflow:hidden;">
<img src="${fn.portfolio_image}"
 alt="Time for new start ? We’re hiring">
</a>
<a href="/inhouse/portfolio/portfolioView/${fn.portfolio_id}" class="fs-14 ml-4"><i class="fas fa-arrow-right"></i></a>
</div>
</div>
</c:forEach>
            ...
            <c:if test="${pageContext.request.userPrincipal.name!=null}">
            <form action="memberinarticle/${portfolio_id}" method="post">
            <input  type="hidden" id="member_name" name="member_name" value="${pageContext.request.userPrincipal.name}">
    ...
    <textarea id="portfolio_in" name="portfolio_in" maxlength=50 minlength=5 class="form-control" style="width:700px;mapping-top:100px;margin-left:400px;height:200px;resize : none;border-radius: 10px;"required="required"></textarea></div>
    <p class="mb-10 text-center"></p>
...
<a href="/inhouse/portfolio/portfolioView/${t3.portfolio_id}" class="hover-shine card-img-top" style="display:inline-block;width:100%;height:250px;">
<img src="${t3.portfolio_image}" alt="Under the sun in Cape Town">
</a>
...
<a href="/inhouse/portfolio/portfolioView/${t3.portfolio_id}">${t3.portfolio_title}</a>
</h3>
</div>
</div>
</div>
...
<span class="fs-12 text-primary">${amin.portfolio_intime}</span>
</p>
<p class="text-gray-01 lh-175 mb-2">${amin.portfolio_in}</p>
<li class="list-inline text-lg-right" style="margin-top:- 20px;">
...
<button class=" btn-primary text-uppercase letter-spacing-05" style="border-radius: 10px;"
 onclick = 'deleteMembername("${amin.idpm}","${member.member_name}")'>
 刪除留言																																		
 </button>
</c:if>
<c:if test="${pageContext.request.userPrincipal.name!=null}">
 <button class=" btn-primary text-uppercase letter-spacing-05" style="border-radius: 10px;"
  onclick = 'golikeportfoliomessage("${amin.portfolio_id}","${member.member_name}","${amin.idpm}")'>
 <i class="far fa-thumbs-up"></i>																																				
 </button>
  <input  type="hidden" id="likenow${amin.idpm}" value="${amin.portfoliomember_like+1}">
</i> <asd id="likeis${amin.idpm}">${amin.portfoliomember_like}Like</a></li>
</c:if>
<c:if test="${pageContext.request.userPrincipal.name==null}">
 <button class=" btn-primary text-uppercase letter-spacing-05" style="border-radius: 10px;"
 onclick = 'golikeportfoliomessage("${amin.idpm}","${member.member_name}")'disabled="disabled">
 <i class="far fa-thumbs-up "></i>																																				
 </button> 
</i>${amin.portfoliomember_like} Like</a></li>
</c:if>
</div>
</div>
</c:forEach>
...
<script type="text/javascript">
<!-----------留言喜歡----oKKKK---->
function golikeportfoliomessage(portfolio_id,member_name,idpm){
    $.ajax({
        url: '/inhouse/portfolio/golikeportfoliomessage/'+ portfolio_id +'/'+member_name+'/'+idpm,
        type: 'POST',
        success: function () {
         	 	var like = document.getElementById("likeis"+idpm);
       	    	var likeas = document.getElementById("likenow"+idpm).value;     
...
<!-------------------->
 ...
 function deleteMembername(portfolio_id,member_name){
     Swal.fire({
         title: "操作確認",
         icon: 'warning',
         text: "您確定要刪除嗎?!",
        ...
        if (result.value) {
     	   Swal.fire({icon: 'success',
           	 title: '留言刪除成功',
           	  ...
           	  timer: 1500});
     	  $.ajax({
              url: '/inhouse/portfolio/delete/' + portfolio_id +'/'+member_name,
              type: 'POST',
              success: function () {
                  <!-----不會刷新-------->
              //    history.go(0)
 ...
 <!-----------作品集喜歡------OK-->
 function articlelike(portfoilo_id,member_name){
     $.ajax({
         url: '/inhouse/portfolio/portfoliolikego/' + portfoilo_id +'/'+member_name,
         type: 'POST',
         success: function () {
        	    Swal.fire('您喜歡這個作品集~');
        	 	var  liketo = document.getElementById("liketo"+portfoilo_id);
        	  	var liketonow = document.getElementById("articles"+portfoilo_id).value;          
